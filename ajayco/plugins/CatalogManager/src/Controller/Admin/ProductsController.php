...
class ProductsController extends AppController
{
    ...
    public function deleteImage(){
        ...
        $data = $this->Products->ProductImages->get($this->request->data['id']);
        $response = [];
        if(!empty($data)){
            $response = [
                'status' => true,
            ...
            $this->Products->ProductImages->delete($data);    
        }
            ...
            if (in_array(strtolower($ext), $allowed)) {
                ...
                if (move_uploaded_file($_FILES['file']["tmp_name"], $dir . $upload_img)) {
                    if (file_exists($dir . $upload_img) && $upload_img != "") {
                        $image = Router::url("/", true) . 'timthumb.php?src=' . $this->request->webroot . "webroot/" . $dir . $upload_img . '&w=100&h=100&a=t&q=100';
                    } else {
                        $image = Router::url("/", true) . 'timthumb.php?src=' . $this->request->webroot . "webroot/" . 'img/noimage.jpg&w=100&h=100&iar=1';
                    }
                    $imagepath = "";
