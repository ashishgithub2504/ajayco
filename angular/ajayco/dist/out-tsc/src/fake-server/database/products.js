...
export const products = productsDef.map(productDef => {
    let badges = [];
    if (productDef.badges) {
        badges = typeof productDef.badges === 'string' ? [productDef.badges] : productDef.badges;
    }
    const categories = shopCategoriesList.filter(x => productDef.categories.includes(x.slug)).map(x => (Object.assign({}, x, { parents: null, children: null })));
...
export function getBestsellers(limit = null) {
    const start = 0;
    const end = limit ? start + limit : undefined;
    return of(products.slice(start, end));
}
export function getTopRated(limit = null) {
    const start = 3;
    const end = limit ? start + limit : undefined;
    return of(products.slice(start, end));
}
export function getSpecialOffers(limit = null) {
    const start = 6;
    const end = limit ? start + limit : undefined;
    return of(products.slice(start, end));
}
export function getFeatured(categorySlug = null, limit = null) {
...
export function getLatestProducts(categorySlug = null, limit = null) {
